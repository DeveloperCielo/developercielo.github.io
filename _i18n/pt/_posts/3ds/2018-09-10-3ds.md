...
Já as transações seguintes (a partir da segunda) não são autenticadas na versão atual do 3DS (3DS 2.2). Assim, se houver chargeback a partir da segunda transação, a responsabilidade é da loja.
...
| Endereço de entrega | `bpmpi_shipTo_shippingmethod` | Tipo do método de envio | lowcost: envio econômico<br>sameday: envio no mesmo dia<br>oneday: envio no dia seguinte<br>twoday: envio em dois dias<br>threeday: envio em três dias<br>pickup: retirada na loja<br>other: outros<br>none: não há envio | Não |
