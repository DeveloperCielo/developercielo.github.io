...
# Novo portal de desenvolvedores 

A partir do dia 24/06/206, nossas documentações passam a estar disponíveis em um novo portal, com navegação mais simples, conteúdos mais organizados e informações técnicas mais completas para apoiar sua integração. 

[Acesse o novo portal de desenvolvedores](docs.cielo.com.br): docs.cielo.com.br.

O novo ambiente foi pensado para facilitar sua jornada como desenvolvedor proporcionando: 

✅ Interface mais intuitiva e moderna; 
✅ Navegação simplificada; 
✅ Referências de API mais completas e estruturadas; 
✅ Maior padronização entre documentações; 
✅ Melhor experiência de leitura, aprendizado e integração. 
...
Caso tenha dúvidas e precise de ajuda, envie um e-mail para **integracaosmart@cielo.com.br**.
...
Para solicitar uma Cielo Smart para testes e publicação de aplicativos, envie um e-mail para **integracaosmart@cielo.com.br**.
...
Durante o envio de requisições via deep link, o uso de parâmetros incorretos ou incompletos pode resultar em falhas. Quando isso ocorre, o sistema retorna mensagens de erro como resposta, com o objetivo de ajudar na identificação e correção do problema. Aqui, reunimos os principais códigos de erro que podem surgir durante o uso de deep links e suas causas.  

### Pagamento

Os erros referentes a pagamentos retornados aparecem no formato Base64, como no exemplo abaixo: 

```html
order://response?response=eyJjb2RlIjoxLCJyZWFzb24iOiJDQU5DRUxBRE8gUEVMTyBVU1XDgVJJTyJ9&responsecode=0
```

Após a decodificação da string Base64, o conteúdo é convertido em um objeto JSON estruturado, que facilita a interpretação do erro. A estrutura será a seguinte:
...
Já se houver algum erro na impressão, o campo response retornará um Base64 com o motivo do erro. 
...
| `merchantCode`  | string  | Não         | Código do estabelecimento. Formato: 00 + (EC = 10 dígitos) + (Loja do EC = 4 dígitos) = (Total de 16 dígitos)                                                 | 0012397373089400 |
...
| `merchantCode`         | string   | Sim         | Código do estabelecimento do subadquirente. Formato: 00+ (EC: 10 dígitos) + (Loja do EC: 4 dígitos) = (Total de 16 dígitos).                                | 0012397373089400             |
...
| `externalCallMerchantCode`  | string        | Número do Estabelecimento Comercial. Formato: 00+ (EC: 10 dígitos) + (Loja do EC: 4 dígitos) = (Total de 16 dígitos)                                       | 0010000244470001                       |
...
| `merchantCode`              | string        | Número do Estabelecimento Comercial. Formato: 00+ (EC: 10 dígitos) + (Loja do EC: 4 dígitos) = (Total de 16 dígitos)                                       | 0010000244470001                       |
