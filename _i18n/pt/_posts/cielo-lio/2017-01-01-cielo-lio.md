...
A API Order Manager da Cielo Smart oferece os seguintes recursos: 
...
Você poderá consultar os dados de entrada, exemplos de requisição e os dados de saída. Assim como entender o fluxo básico da utilização da API Order Manager da Cielo Smart.
...
Todas as chamadas da API Integração Remota Cielo Smart Order Manager necessitam que as informações abaixo estejam presentes no Header na requisição:

- **Client-Id**: Identificação de acesso. Sua geração ocorre no momento da criação pelo painel do desenvolvedor. 
- **Access-Token**:Identificação do token de acesso, que armazena as regras de acesso permitidas ao Client ID.  
- **Merchant-Id**: Token que identifica o estabelecimento comercial dentro do servidor Order Manager da Cielo LIO. 
