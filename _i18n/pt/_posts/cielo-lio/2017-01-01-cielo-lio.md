...
| `installments`  | inteiro  | Sim         | Número de parcelas (0 para pagamento à vista)                            |
| `items`         | array    | Sim         | Lista de itens da compra com nome, quantidade, SKU, unidade de medida e valor     |
| `paymentCode`   | string   | Sim         | Meio de pagamento                                              |
...
| Campo                  | Tipo     | Obrigatório | Descrição                                                                 |
|------------------------|----------|-------------|---------------------------------------------------------------------------|
| `softDescriptor`         | string   | Não         | Texto que será exibido na fatura do cliente                              |
| `terminalId`             | string   | Não         | Identificador do terminal de pagamento                                   |
| `merchantCode`           | string   | Não         | Código do estabelecimento do subadquirente                               |
| `city`                   | string   | Não         | Cidade do subadquirente                                                  |
| `telephone`              | string   | Não         | Telefone de contato do subadquirente                                     |
| `state`                  | string   | Não         | Estado do subadquirente                                                  |
| `postalCode`             | string   | Não         | Código postal (CEP) do subadquirente                                     |
| `address`                | string   | Não         | Endereço completo do subadquirente                                       |
| `identifier`             | string   | Não         | Identificador único do subadquirente                                     |
| `merchantCategoryCode`   | string   | Não         | Código da categoria do estabelecimento                                   |
| `countryCode`            | string   | Não         | Código do país (formato ISO 3166-1 alfa-2, ex: "BR")                     |
| `informationType`        | string   | Não         | Tipo de informação adicional fornecida                                   |
| `document`               | string   | Não         | Documento de identificação do subadquirente (ex: CNPJ ou CPF)           |
| `businessName`           | string   | Não         | Nome comercial do subadquirente                                          |
| `ibgeCode`               | string   | Não         | Código IBGE do município do subadquirente                                |
