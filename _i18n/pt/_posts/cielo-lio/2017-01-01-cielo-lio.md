...
**3**. Selecione a API **Integração Local** para realizar a integração via Deep link.  
...
- **Certificado alterado**: A submissão do aplicativo é rejeitada quando há mudança na assinatura digital (certificado) em relação à versão anteriormente cadastrada. Para solucionar esse erro, verifique se o certificado atual é o mesmo utilizado nas versões anteriores. Se houver alteração no targetSdkVersion, é necessário seguir as orientações específicas para garantir que a assinatura seja compatível com a nova versão do SDK. Isso ocorre porque versões mais recentes do Android exigem certificados que atendam aos novos padrões de segurança, como o APK Signature Scheme v3 ou v4. 
