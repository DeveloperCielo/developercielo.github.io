...
A seção [Pré-publicação de aplicativo](https://developercielo.github.io/manual/cielo-lio#publicando-um-aplicativo), reúne os principais pontos que devem ser considerados antes de iniciar o processo de publicação de um app. Ela inclui os [pré-requisitos técnicos](https://developercielo.github.io/manual/cielo-lio#pr%C3%A9-requisitos), os [modelos de integração disponíveis](https://developercielo.github.io/manual/cielo-lio#modelos-de-integra%C3%A7%C3%A3o), e orientações sobre o uso do [Emulador Cielo](https://developercielo.github.io/manual/cielo-lio#emulador-cielo). Também aborda como [solicitar e ativar uma Cielo Smart](https://developercielo.github.io/manual/cielo-lio#solicitando-uma-cielo-smart), além de listar recursos indisponíveis e sugerir alternativas viáveis para garantir uma experiência completa e funcional no aplicativo.

Já em [Publicando um aplicativo](https://developercielo.github.io/manual/cielo-lio#publicando-um-aplicativo), o foco é no processo de criação, testes e publicação de aplicativos. Você aprende desde os primeiros passos no desenvolvimento até o uso do Cielo Store Dev Console, ferramenta que permite gerenciar os aplicativos dentro da plataforma. Também são explicadas as diferenças entre loja pública e loja privada, como acompanhar o status do aplicativo, os critérios para publicação e como utilizar o ambiente Sandbox, que funciona como um emulador para testes sem a necessidade de um terminal físico.  

A seção [Integração via Deeplink](https://developercielo.github.io/manual/cielo-lio#integracao-via-deeplink5) apresenta uma forma simples e eficiente de conectar seu aplicativo à Cielo Smart por meio de deep links. São fornecidos exemplos práticos, formatos de chamada e orientações para garantir compatibilidade e segurança na comunicação.

A seção [Integração Remota](https://developercielo.github.io/manual/cielo-lio#integra%C3%A7%C3%A3o-remota) detalha como realizar transações e interações com a Cielo Smart de forma remota. O conteúdo inclui fluxos de integração, requisitos técnicos e boas práticas para garantir confiabilidade e desempenho.

Na seção [Pix](https://developercielo.github.io/manual/cielo-lio#pix), você encontra orientações para integrar pagamentos via Pix na Cielo Smart, oferecendo uma alternativa moderna e instantânea às transações tradicionais. 

Por fim, a seção de [Perguntas Frequentes](https://developercielo.github.io/manual/cielo-lio#faq) reúne as dúvidas mais comuns entre os usuários da plataforma. 
...
**3**. Selecione a API **Integração Local** para realizar a integração via deeplink.  

**4**. Clique em **Registrar** para concluir o processo. 

![Registrando credenciais](../../../../images/cielo-lio/credenciaisintegracaolocal.png)
...
Nossa integração funciona sem depender do SDK, basta criar uma intent e passá-la como parâmetro no método `startService`, que pode ser acessado via o context da aplicação.
 ...
 **1. Crie um serviço em primeiro plano**: Implemente um serviço que seja executado em primeiro plano com uma notificação persistente.

 **2. Inicie o serviço em primeiro plano**: Inicie o serviço antes de lançar o deep link.

 **3. Lance o deep link**: Use um Intent para iniciar o outro aplicativo através do seu deep link.
...
<aside style="background-color: #d4edda; padding: 15px; border-left: 5px solid #28a745; color: #155724;margin-top: 20px; margin-bottom: 20px;"> As credenciais de autenticação da integração remota funcionam apenas no ambiente Sandbox. Para usar em produção, será preciso gerar o token (merchant-ID) de cada estabelecimento comercial (EC), já que cada um tem seu próprio identificador. Para gerar um merchant-ID, entre em contato com o time de suporte e faça sua solicitação. </aside>
...
- Realizar testes e desenvolver sua aplicação utilizando emuladores ou ambientes sandbox. <br/>
<br/>
