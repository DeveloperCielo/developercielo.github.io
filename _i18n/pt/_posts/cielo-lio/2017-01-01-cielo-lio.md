...
Após a criação de conta no Dev Console, deverá fazer um pedido de LIO através da abertura de solicitação no Portal de Desenvolvedores > Suporte > Cielo Lio > Envie sua pergunta > Solicitação LIO DEV / Credenciamento EC Cielo.
...
- Termos de uso deve ser preenchido corretamente, mostrando as normas do serviço que está sendo oferecido.
- O formulário dos dados do aplicativo deve ser preenchido corretamente.
- É necessário que informe os dados para Suporte do aplicativo
- Caso o aplicativo que contenha vídeo a URL deve ser com o seguinte formato: https://www.youtube.com/embed/
- Imagem do ícone e screenshot no formato correto
...
- Para atualização de versão do app deve possuir Version code superior a última versão enviada anteriormente e manter o mesmo Package name do app das versões anteriores
...
- Realizar testes em desenvolvimento antes de nos encaminhar a versão para atualização. Veja como baixar o Emulador (Link
- A solução pode ser customizada para cliente específico.
- Caso o APP tenha webview, não ficar disponível a URL para a troca de URL dentro do APP. A URL não deverá aparecer.
- Caso o APP rode em um servidor local, enviar um vídeo mostrando o fluxo do APP. Vídeos deverão ser atualizados conforme enviarem uma nova versão para certificação. Após gravar o vídeo do APP, mostrar a versão e data do vídeo. Todo vídeo deverá ser enviado via YouTube.
- Garantir que o fluxo de pagamento funcione corretamente.
- Gerar valores para teste até R$0,05 centavos.
- 100% das transações dever ser via Cielo.
...
- Realizar testes em desenvolvimento antes de nos encaminhar a versão para atualização. Veja como baixar o [Emulador](https://developercielo.github.io/manual/cielo-lio#ambiente-sandbox-%E2%80%93-emulador).
- Caso o APP tenha webview, não ficar disponível a URL para a troca de URL dentro do APP. A URL não deve aparecer.
- Garantir que o fluxo de pagamento funcione corretamente.
- Gerar valores para teste até R$0,05 centavos.
- 100% das transações dever ser via Cielo
-  Para atualização de versão do app deve possuir Version code superior a última versão enviada anteriormente e manter o mesmo Package name do app das versões anteriores
...
- Caso o APP rode em um servidor local, envie um vídeo mostrando o fluxo do APP. Vídeos deverão ser atualizados conforme enviarem uma nova versão para certificação. Após gravar o vídeo do APP, mostrar a versão e data do vídeo. Todo vídeo deverá ser enviado via YouTube.
- Garantir que o aplicativo seja seguro para todos os públicos.


**Dicas para Vídeo para etapa de Certificação**

Para garantir um processo de certificação eficaz, é fundamental que o vídeo enviado capture todos os detalhes da versão do aplicativo em questão. Siga estas orientações para garantir que seu vídeo atenda aos requisitos: 

**Gravação na Horizontal:** Sempre grave na orientação horizontal para garantir melhor visibilidade da tela.

- **Tela Completa e Comprovante:** Certifique-se de que a câmera capture toda a tela do dispositivo e tenha espaço suficiente para mostrar a saída do comprovante. 
- **Versão do Aplicativo:** No início do vídeo, mostre claramente o número da versão que está sendo homologada. 
- **Navegação pelo Aplicativo:** Demonstre a navegação pelo aplicativo, mas lembre-se de que não deve haver referências ou logotipos da empresa Cielo. 
- **Saída do Comprovante:** Filme a saída do Comprovante, garantindo que o valor e a data estejam claramente visíveis.  

Seguindo esses passos, você agiliza o processo de certificação, **especialmente para aplicativos onde o time de homologação não temos acesso direto.** Isso garante que todas as etapas de certificação sejam realizadas de forma rápida e eficaz.
...
- Não exibir de imagens ou nomes de concorrentes.
- Não exibir Logo da Cielo. 
...
![checkoutorder formas](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/images/ProcessoDePagamento2_0.png)
...
![status do pedido](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/images/StatusDePedido.png)
...
![fluxo_basico](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/images/APIOrderManagerCielo.png)
