...
O desenvolvedor consegue visualizar todos os seus aplicativos publicados sejam eles aplicativos privados ou públicos.

Todo aplicativo passa por certificação de um time especializado antes de ser disponibilizado na Cielo Store (Aplicativos de Loja Pública) ou Distribuído para EC’s associados (Aplicativos de Loja Privada). 

[Clique aqui](https://www.cieloliostore.com.br/login) para se cadastrar no Dev Console.

## LIO para Desenvolvedores

Os desenvolvedores que desejam obter uma Cielo LIO, devem se credenciar no [Portal dos Desenvolvedores](https://desenvolvedores.cielo.com.br/api-portal/) e fazer a solicitação do equipamento através da abertura do ticket.

Após o recebimento da Cielo LIO, o desenvolvedor deve abrir uma nova solicitação no portal dos desenvolvedores para requisitar a habilitação de sua LIO de Dev.

Todos os aplicativos publicados pelo desenvolvedor no Dev Console que estejam com status de Desenvolvimento poderão ser baixados em sua LIO de Dev.
...
[Clique Aqui](https://developercielo.github.io/manual/cielo-lio#cielo-store) para conhecer mais sobre a Cielo Store.

### Upload de novos Aplicativos Públicos e novas versões

Acesse o Dev Console para realizar o upload de um aplicativo de Loja Pública.

Para realizar o upload de um aplicativo de loja pública siga as instruções abaixo:

- Entre no DevConsole e selecione a opção “+ Aplicativos”. Ao clicar nessa opção irá apresentar o seguinte pop-up mostrando a diferença dos tipos de lojas.

![NOVO APLICATIVO](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/Novo_Aplicativo.png)

- Selecione a opção Loja Pública e clique em “Subir App”. Irá apresentar uma mensagem no qual esteja concordando que **“Após o upload do arquivo não é possível trocar de loja pública para privada.”**

- Suba o arquivo do aplicativo e aperte em salvar.

- Envie o app para certificação. Preencha o formulário dos dados do aplicativo corretamente. Após enviar o app para certificação e preencher os formulários o aplicativo será certificado por um time especializado. Lembrando que antes de colocar o app para etapa de certificação o aplicativo deve ser testado pelo dev.

- Aplicativo sendo aprovado. O parceiro será notificado através do Dev Console, sendo assim poderá colocar seu app em produção. O aplicativo após ser promovido para produção está disponível na Vitrine da Cielo Store.

- Caso o aplicativo seja reprovado o time especializado irá devolver uma mensagem informando o motivo da reprovação. Essa mensagem irá ficar disponível no Dev Console mediante a versão enviada. Portanto, o desenvolvedor deverá realizar a correção e subir uma nova versão no Dev Console e promover o aplicativo para certificação novamente.

**Atenção:** Caso o desenvolvedor esteja realizando uma alteração ou atualização no aplicativo público, o desenvolvedor deverá fazer o upload de uma nova versão do aplicativo. Para isso clique em Detalhes do seu aplicativo e inicie o procedimento para adicionar uma nova versão. Para enviar uma nova versão do aplicativo é obrigatório que:
...
Como funciona a compra de aplicativos de loja pública:

- Entre na Vitrine Cielo Store.
- Escolha o aplicativo de sua preferência.
- Clique em instalar. Irá apresentar uma tela solicitando os dados de acesso. Os dados são os mesmos do site Cielo.
- Aplicativo pronto para ser utilizado. 

> **Atenção:** Todos os aplicativos na primeira compra o primeiro mês é gratuito.
...
[Clique Aqui](https://devcielo.zendesk.com/hc/pt-br) para solicitar a criação da Loja Privada.

### Upload de novos Aplicativos e novas versões na loja privada

Acesse o Dev Console para realizar o upload de um aplicativo de Loja Privada.

Para realizar o upload de um aplicativo de loja privada siga as instruções abaixo:

- Entre no DevConsole e selecione a opção “+ Aplicativos”. Ao clicar nessa opção irá apresentar o seguinte e pop-up mostrando a diferença dos tipos de lojas.

![NOVO APLICATIVO](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/Novo_Aplicativo.png)

- Selecione a opção Loja Privada e clique em “Subir App”. Irá apresentar uma mensagem no qual esteja concordando que “Após o upload do arquivo não é possível trocar de loja privada para pública. 

- Suba o arquivo do aplicativo e selecione uma loja para **associar seu aplicativo**. Caso não tenha uma loja criada. Solicite no portal do desenvolvedor.

- Envie o app para certificação. Após enviar o app para certificação e preencher os formulários o aplicativo será certificado por um time especializado. Nesta loja privada, o desenvolvedor deve enviar os dados de acesso para o app ser testado. Lembrando que antes de colocar o app para etapa de certificação o aplicativo deve ser testado pelo dev.

- Aplicativo sendo aprovado. O parceiro será notificado através do Dev Console, sendo assim poderá colocar seu app em produção. O aplicativo após ser promovido para produção está disponível na Vitrine da Cielo Store.

- Caso o aplicativo seja reprovado o time especializado irá devolver uma mensagem informando o motivo da reprovação. Essa mensagem irá ficar disponível no Dev Console mediante a versão enviada. Portanto, o desenvolvedor deverá realizar a correção e subir uma nova versão para ser certificada no DevConsole e promover o aplicativo para certificação novamente.
...
- Desenvolvedor deverá promover para produção seu aplicativo [Clicando aqui](https://developercielo.github.io/manual/cielo-lio).

- Após o aplicativo ser promovido para produção ele será disponibilizado para os EC’s associados a loja.

> **Atenção:** Somente aplicativos de loja pública são disponibilizados na Cielo Store.

### Distribuição de Aplicativos na loja privada

Aqueles desenvolvedores que optarem por desenvolver aplicativos privados e distribuir apenas para clientes específicos devem efetuar o vínculo do EC do cliente a sua loja privada, pelo próprio Dev Console. O próprio desenvolvedor que gerencia essa inclusão do EC do cliente a loja desejada.
...
Para o aplicativo ser aprovado com sucesso na fase de Certificação leia todos os itens abaixo: 

**Critérios Loja Pública:**
...
- Deve ficar disponível a todos os usuários.
...
- Informar a versão do OS utilizada no formulário.
- Informar as funções do SDK utilizadas pelo APP, Changelog do APP (mudanças feitas para a versão que está sendo enviada para certificação) no formulário.
...
- O aplicativo deve deixar o cliente realizar o cadastro facilmente.
...
**Critérios Loja Privada:**

- A solução pode ser customizada para um cliente específico.
- Acesso restrito e personalizado a um cliente.
- A versão a ser certificada deve ser maior do que a versão anterior. EX: versão anterior 1.0 / versão atual 1.1
- Caso o APP tenha webview, não ficar disponível a URL para a troca de URL dentro do APP. A URL não deverá aparecer.
- Caso o APP rode em um servidor local, enviar um vídeo mostrando o fluxo do APP. Vídeos deverão ser atualizados conforme enviarem uma nova versão para certificação. Após gravar o vídeo do APP, mostrar a versão e data do vídeo. Todo vídeo deverá ser enviado via youtube.com
- Garantir que o fluxo de pagamento funcione corretamente.
- Gerar valores para teste até R$0,05 centavos
- 100% das transações dever ser via Cielo.
- Informar a versão do SDK utilizada no formulário.
...
- Selecionar um aplicativo irá exibir a tela de versões do app escolhido.
...
- Nesse momento, basta abrir o aplicativo Test Your App, que estará na Lio de dev, e ler esse QR Code. 

![QR Code](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/qrcode.png)

- Após fazer a leitura o app irá baixar na Cielo LIO.

> **Atenção:** Para instalação de um app na Cielo Lio, o terminal deverá estar com bateria acima de 50% ou conectado no cabo de energia e internet 3G ou Wi-FI.
...
## Informações de Compra do Seu App
...
Efetue o download da nossa SDK [CLIQUE AQUI](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/ctools/SDK-1.5.13.zip)

Disponilizamos dois diretorios `com` e `cielo` basta copiá-los para o seu repositório `.m2/repository`, exemplos:

No Windows: `C:\Users\<usuario>\.m2`
No Linux: `/home/<usuario>/.m2`
No Mac: `/Users/<usuario>/.m2`

No **build.gradle** na raiz do seu projeto, adicione o `mavenLocal()` dentro da closure **allprojects** conforme abaixo:
...
Acesse o nosso projeto de exemplo em: [React Native](https://github.com/DedoJr/cieloReturnPayments)
...
[![Vídeo Demo](https://i.imgur.com/kUa1iYf.png)](https://youtu.be/ftS3QKlCX4U){:target="\_blank"}
