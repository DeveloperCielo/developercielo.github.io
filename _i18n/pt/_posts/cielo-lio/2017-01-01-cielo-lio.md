...
### Últimas atualizações: L400 - CJ19 
...
- Relatório Pix por horário para análise detalhada. 

**Relatórios e análises** 

- Inclusão de relatório por período para melhor acompanhamento. 
- Exibição de valores e quantidades no relatório detalhado. 

**QR Code** 

- Geração de QR Code na Função 11 – Dados do EC. 
- Impressão de QR Code no resultado da Função 87 – Script de Testes. 
...
- Prevenir que transações offline recusadas sobrescrevam dados de transações online. 
...
- Implementação da Onda II do BIT47, com envio estruturado de dados ao backend para apoiar modelos e análises logísticas.
...
- Ajuste no fluxo de desbloqueio de terminal bloqueado via sinalização do autorizador. 

**Tratamento de dados e arquivos** 

- Ajuste no processo de armazenamento de imagens para impressão via app integrado. 
- Alteração no formato de salvamento do APK, removendo caracteres especiais do caminho dos arquivos para evitar erros.  
...
- A via do estabelecimento passa a ser opcional e não será impressa automaticamente. 
...
- Alteração no fluxo da função 86 (Teste de Leitura de Cartão). 

**Conectividade** 

Substituição do modo avião pela função "Reiniciar comunicação”, que realiza o mesmo processo. 

**Performance e estabilidade** 

Prevenção de vazamentos de memória usando WeakReference para Activity e limpeza do listener de exceção no método onDestroy. 
...
- Ajuste na ativação do serviço Cashless via URI App. 
...
- A opção “Reiniciar comunicação” no app Configurações atualmente não apresenta feedback ao usuário, mas já está em processo de melhoria.

## Outubro de 2025

### Últimas atualizações: DX800 - CH19 
...
- Interface: Inclusão de atalhos de formas de pagamento na tela inicial. 
...
- Ajuste no campo de data da transação. 
...
- Correção na gravação do Merchant-ID utilizado nas requisições ao Order.

### Últimas atualizações: L300 (SO, Configurações e Serviço) 

**Novidades** 

Adicionamos novas funcionalidades para melhorar o desempenho e a usabilidade. Confira as novidades desta atualização.

- Início da venda pelo cartão: Agora é possível iniciar a venda assim que o cartão for inserido no App Pagamento. 
- Monitoramento do SO: A versão do sistema operacional pode ser acompanhada via ferramenta de Observabilidade. 

**Melhorias**: 

Esta atualização também inclui aprimoramentos de recursos existentes, melhorando o desempenho e a eficiência. 

- Nova opção para reiniciar comunicação: Substitui o modo avião pela função “Reiniciar comunicação”, que realiza o mesmo processo. 

**Correções de bug** 

Esta atualização corrige falhas para aprimorar a estabilidade e a confiabilidade. 

- Corrigido erro na impressão via aplicativo integrado usando DeepLink/UriApp. 
- Corrigido erro na ativação do Cashless via integração DeepLink/UriApp. 
