...
<aside style="background-color: #f8d7da; padding: 15px; border-left: 5px solid #dc3545; color: #721c24;margin-top: 20px; margin-bottom: 20px;"> A partir do Android 11, os aplicativos não podem mais acessar o diretório Android/data/ de outros apps, mesmo que o usuário tenha concedido permissão de armazenamento. Por isso, evite utilizar caminhos como: `/storage/emulated/0/Android/data/package.name/files/`. Esse tipo de acesso é restrito pelo sistema e pode causar falhas no funcionamento do aplicativo. Para salvar imagens ou outros arquivos, recomenda-se utilizar o diretório: `/storage/emulated/0/saved_images/`. </aside>
