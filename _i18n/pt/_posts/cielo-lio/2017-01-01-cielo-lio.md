...
- Apoio em feiras e eventos do setor.<br/>
<br/>
...
-  Em caso de dúvidas, abra um chamado no portal ou envie e-mail para
   atendimentosmart@cielo.com.br <br/>
   <br/>
...
| **Densidade de pixels**   | 267 ppi                     | 1080p                       | 269,92 ppi             |
...
| **Sensores**              | Acelerômetro, Prox. e Lumin | Acelerômetro, Prox. e Lumin | NA             |
...
- Boa opção para soluções mais leves ou apps móveis. <br/>
<br/>
...
- Ideal para quem já possui uma solução robusta de automação comercial. <br/>
<br/>
...
**1**. Acesse no menu **Lojas Privadas**. <br/>
...
| Chave  | Descrição                        |
|--------|----------------------------------|
| code   | Código numérico de erro          |
| reason | Mensagem de erro com justificativa |
...
|---------------|----------|-------------|---------------------------------------------------------------------------|
| `accessToken`   | string   | Sim         | Token de acesso fornecido pela Cielo para autenticação                   |
| `clientID`      | string   | Sim         | Identificador do Cliente na plataforma da Cielo                          |
| `reference`     | string   | Não         | Referência do pedido, utilizada para controle interno                    |
| `merchantCode`  | string   | Não         | Código do estabelecimento                                                |
| `email`         | string   | Sim         | E-mail do cliente                                                        |
| `installments`  | string  | Sim         | Número de parcelas (0 para pagamento à vista)                            |
| `items`         | array    | Sim         | Lista de itens da compra com nome, quantidade, SKU, unidade de medida e valor     |
| `paymentCode`   | string   | Sim         | Meio de pagamento. Veja abaixo os valores aceitos no campo paymentCode                                              |
| `value`         | string   | Sim         | Valor total da transação em centavos (ex: "1000" para R$10,00)          |
...
| Campo                  | Tipo     | Obrigatório | Descrição                                                                 |
|------------------------|----------|-------------|---------------------------------------------------------------------------|
| `softDescriptor`         | string   | Sim         | Texto que será exibido na fatura do cliente                              |
| `terminalId`             | string   | Sim         | Identificador do terminal de pagamento                                   |
| `merchantCode`           | string   | Sim         | Código do estabelecimento do subadquirente                               |
| `city`                   | string   | Sim         | Cidade do subadquirente                                                  |
| `telephone`              | string   | Sim         | Telefone de contato do subadquirente                                     |
| `state`                  | string   | Sim         | Estado do subadquirente                                                  |
| `postalCode`             | string   | Sim         | Código postal (CEP) do subadquirente                                     |
| `address`                | string   | Sim         | Endereço completo do subadquirente                                       |
| `identifier`             | string   | Sim         | Identificador único do subadquirente                                     |
| `merchantCategoryCode`   | string   | Sim         | Código da categoria do estabelecimento                                   |
| `countryCode`            | string   | Sim         | Código do país (O código do país do Brasil é 0076)                       |
| `informationType`        | string   | Sim         | Tipo de informação adicional fornecida                                   |
| `document`               | string   | Sim         | Documento de identificação do subadquirente (ex: CNPJ ou CPF)           |
| `businessName`           | string   | Sim         | Nome comercial do subadquirente                                          |
| `ibgeCode`               | string   | Sim         | Código IBGE do município do subadquirente                                |
...
|                             | **PaymentCode**                 |                             |
|-----------------------------|----------------------------------|-----------------------------|
| DEBITO_AVISTA               | CREDITO_AVISTA                  | VOUCHER_ALIMENTACAO         |
| DEBITO_PAGTO_FATURA_DEBITO  | CREDITO_PARCELADO_LOJA          | VOUCHER_REFEICAO            |
| CREDITO_PARCELADO_ADM       | CREDITO_PARCELADO_BNCO          | VOUCHER_AUTOMOTIVO          |
| PRE_AUTORIZACAO             | CREDITO_PARCELADO_CLIENTE       | VOUCHER_CULTURA             |
| CREDITO_CREDIARIO_CREDITO   | VOUCHER_PEDAGIO                 | VOUCHER_BENEFICIOS          |
| VOUCHER_AUTO                | VOUCHER_CONSULTA_SALDO          | VOUCHER_VALE_PEDAGIO        |
| CREDIARIO_VENDA             | CREDIARIO_SIMULACAO             | CARTAO_LOJA_AVISTA          |
| CARTAO_LOJA_PARCELADO_LOJA  | CARTAO_LOJA_PARCELADO           | CARTAO_LOJA_PARCELADO_BANCO |
| CARTAO_LOJA_PAGTO_FATURA_CHEQUE | CARTAO_LOJA_PAGTO_FATURA_DINHEIRO | FROTAS                 |
| PIX                         |                                 |                             |
...
<aside style="background-color: #d4edda; padding: 15px; border-left: 5px solid #28a745; color: #155724;margin-top: 20px; margin-bottom: 20px;"> O campo statusCode=0 (apenas pagamentos pix) ou 1 indica uma transação de pagamento, enquanto o campo statusCode=2 indica um Cancelamento. </aside> 
...
<aside style="background-color: #f8d7da; padding: 15px; border-left: 5px solid #dc3545; color: #721c24;margin-top: 20px; margin-bottom: 20px;"> A princípio, será possível recuperar, no máximo, cinco pedidos devido ao tamanho das informações. </aside> 
...
É possível obter diversas informações do terminal, como o nível da bateria, o modelo do dispositivo, o Merchant e o número lógico. Para isso, basta realizar a chamada da seguinte forma:
...
<aside class="warning"> Não é possivel realizar cancelamentos de pagamentos efetuados por PIX via integração.</aside>
...
A Cielo Smart oferece a solução de pagamento cashless via integração. Suportamos os cartões Mifare 1k. Para utilizar o serviço no aplicativo de integração, comece adicionando no arquivo AndroidManifest.xml a seguinte permissão:
...
Os cartões Mifare possuem uma pequena antena que armazena dados e permite a comunicação com os leitores quando estão próximos. Atualmente, damos suporte ao modelo Mifare 1k, que tem a memória dividida em setores e blocos: cada setor possui 4 blocos, e cada bloco armazena até 16 bytes, totalizando 64 bytes de dados.

Nossa integração funciona sem depender do SDK da Lio, basta criar uma intent e passá-la como parâmetro no método `startService`, que pode ser acessado via o context da aplicação.
...
Para realizar operações com cartão Mifare, basta o integrador criar uma *intent* seguindo nossa especificação e utilizá-la como parâmetro do método `startService`, disponível na classe context.
...
Operação responsável por ativar a antena e aguardar a aproximação de um cartão. Em caso de sucesso na operação, a intent de retorno conterá o parâmetro `result`, que é uma String com o valor `R00`, e o parâmetro `data`, que é um ByteArray contendo o identificador do cartão. 

Se a operação resultar em erro, o parâmetro `result` retornará uma String com um valor diferente de `R00`, e o identificador do cartão não será retornado no parâmetro `data`.
...
Os componentes responsáveis por tratar os retornos das operações descritas anteriormente podem variar, como broadcasts, services ou intents. Apesar das diferenças, todos compartilham um ponto em comum: o uso de uma intent como parâmetro. Essa intent deve conter os seguintes atributos:
...
A seguir, apresentamos uma classe de exemplo implementada em Kotlin que realiza o consumo da API do Mifare. Para essa demonstração, optamos por utilizar o modo de consumo via broadcast.
...
<aside style="background-color: #d4edda; padding: 15px; border-left: 5px solid #28a745; color: #155724;margin-top: 20px; margin-bottom: 20px;"> Para permitir o salvamento de arquivos na DX8000, é necessário incluir o atributo android:requestLegacyExternalStorage="true" na tag <application> do arquivo de manifesto do Android. </aside> 
...
Em plataformas diferentes da LIO, ao realizar a impressão de imagens, pode ocorrer de parte da imagem permanecer dentro da impressora após o término da impressão. Para evitar esse comportamento, é possível utilizar o atributo **form_feed** dentro da propriedade styles. Se esse parâmetro não for especificado, o valor padrão será 0, o que significa que não haverá espaço adicional após a imagem. Para garantir que a imagem seja completamente ejetada da impressora, basta definir o valor como 1. [Consulte exemplos na seção Mapa de estilos](https://developercielo.github.io/manual/cielo-lio#mapa-de-estilos-de-impress%C3%A3o).
