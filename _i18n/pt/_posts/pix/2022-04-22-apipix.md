...
E-mail: sustentacaopix@cielo.com.br <br/>
...
O Pix Automático é uma solução para facilitar pagamentos recorrentes. Com ele, o usuário pagador precisa apenas realizar uma autorização prévia para que o pagamento seja efetuado automaticamente na frequência previamente combinada, podendo ser utilizado em contas como aluguéis, mensalidades, contas de consumo, streamings, seguros e qualquer outra conta que tenha uma periodicidade recorrente de pagamento. 

Esta solução permite que o contrato com o usuário pagador seja indeterminado, sem data limite para fim, ou com período fixo, onde o recebedor indica data de início e de fim da recorrência. Também é necessário informar a periodicidade acordada entre usuário pagador e recebedor, podendo ser: semanal, quinzenal, mensal, trimestral ou anual. 
...
- **Recorrência:** É o ponto de partida para habilitar o recebimento de pagamentos pelo produto. Nessa etapa, é criado um contrato de recorrência que reúne as informações do recebedor, a descrição do débito recorrente, a periodicidade e a data de início das cobranças. Após a criação, é gerado um identificador para essa recorrência (idRec). 

- **Solicitação de recorrência:** Após a criação da recorrência, é necessário que o pagador realize o consentimento para autorizar pagamentos vinculados ao contrato estabelecido. e pagamento da recorrência criada. 

- **Cobrança:** Com o aceite da solicitação de recorrência, o usuário recebedor pode enviar as cobranças correspondentes ao período vigente para agendamento e liquidação. Para isso, cada cobrança deve obedecer as condições definidas no contrato de recorrência, como valor, periodicidade e data de vencimento, além de ter um Txid único por transação. 

- **Cancelamento:** Pode acontecer a qualquer momento do ciclo de vida do produto e pode partir tanto do usuário recebedor como do pagador.
...
No Pix Automático na Cielo, estão previstas quatro jornadas de adesão (autorização da recorrência), além das jornadas de cobrança (agendamento) e do envio de notificações por webhook. 
...
Na Cielo, as jornadas de adesão 1, 2 e 3 já estão disponíveis para os usuários recebedores por meio da API Pix. A jornada 4 ainda está em desenvolvimento para futura disponibilização. 

#### Jornada 1: Por meio de notificação

O usuário pagador escolhe o Pix Automático como forma de pagamento em ambiente da empresa recebedora e informa agência e conta em que o recebedor solicitará para seu PSP recebedor a criação da recorrência e solicitação dela.  

Quando o usuário recebedor solicitar a criação de uma recorrência a Cielo irá devolver um ID de recorrência (IdRec), em que a empresa recebedora poderá utilizar para consultar aquele contrato a qualquer momento. Após é criada uma solicitação de recorrência atrelada ao IdRec que também possui um código único (IdSolicRec)para consulta de solicitação e possui validade de acordo com a data de vencimento que o usuário recebedor informar, conforme fluxograma abaixo: 

![Jornada 1](../../../../images/api-pix/jornada1.png)

#### Jornada 2: Leitura de QRCode 

O usuário pagador escolhe a forma de pagamento como Pix automático e a empresa apresenta para ele um QRCode que contém as informações da recorrência, quando o cliente pagador ler esse QRCode no APP do banco em que possui conta ele será direcionado para a jornada para conceder a autorização dentro do aplicativo do banco.  

Para essa jornada a empresa não precisa ter a informação de agência e conta do pagador, pois essa informação será adicionada a recorrência quando ele aceitar a solicitação no seu banco.  
A solicitação de recorrência pelo QRCode se dá pela rota de location da recorrência com um ID específico das rotas com leitura de QRCode. A forma de criação da recorrência se mantém a mesma da jornada 1 com mesmo formato de ID, conforme fluxograma abaixo: 

![Jornada 2](../../../../images/api-pix/jornada2.png)

#### Jornada 3: Pagamento imediato de Pix + autorização da recorrência 

A empresa recebedora disponibiliza para seu cliente um QR Code (ou copia e cola) contendo as informações da autorização da recorrência e as informações relativas ao pagamento imediato da primeira cobrança, sendo que nessa jornada concede a autorização ao mesmo tempo em que inicia o pagamento imediato de Pix. 

Quando cliente pagar essa cobrança a solicitação do Pix automático já é aceita automaticamente e as próximas cobranças aconteceram de acordo com o definido na recorrência. 

Importante, caso ocorra algum erro no pagamento imediato do QR Code o fluxo não segue para a autorização. Dado que é um QR Composto com os dois fluxos, conforme fluxograma abaixo:

![Jornada 3](../../../../images/api-pix/jornada3.png)

### Jornada de cobrança: Agendamento  

Para agendar uma cobrança dentro do Pix Automático a empresa recebedora precisa enviar o agendamento de acordo com a periodicidade estabelecida no contrato de recorrência. O envio da solicitação de agendamento deve acontecer de 10 a 2 dias antes de vencimento da cobrança, prazo estipulado pelo Banco Central. 

O cancelamento do agendamento pode partir tanto do usuário pagador como do recebedor, mas com regras especificas para cada. O recebedor pode cancelar um agendamento até as 22h00 do dia anterior a data de liquidação e o pagador até as 23h59 do dia anterior a data de liquidação.  

Consulta de agendamento e cancelamento pode acontecer em endpoints específicos para esse serviço, onde a empresa recebedora pode consultar status referente ao agendamento, podendo ser consulta em lista ou individual. 

Na chamada do endpoint CobR, é necessário que o usuário recebedor informe sua agência e conta de pagamento na Cielo. Como não há um campo para o ISPB, não é possível informar uma conta de outro PSP — ou seja, a conta utilizada será sempre a da Cielo. 

Vale lembrar que essa conta não é o domicílio final de pagamento dos valores do Pix. O domicílio bancário será definido conforme a modalidade de recebimento escolhida pelo cliente, conforme descrito anteriormente neste documento. 

Abaixo, segue um exemplo com os campos exigidos. 
...
No Site Cielo, em ambiente logado, acesse a aba Pix e serão apresentados os dados da Conta de pagamento, conforme abaixo: 

![cliente acessando dados](../../../../images/api-pix/acessodados.png)

## Fluxos de webhook

Aplicação que monitora de forma ativa todo os eventos relacionados ao Pix Automático, seja ele iniciado pelo pagador ou pelo recebedor, para notificar o usuário recebedor em relação a cada tapa do fluxo do produto Pix Automático. 

A Cielo disponibilizará o serviço para as etapas de recorrência e todos os passos que ela compõe, e para a jornada de agendamento, que vai até a etapa de liquidação do serviço.

![fluxowebhook](../../../../images/api-pix/fluxodewebhook.png)

## QR Code composto

Nas jornadas que utilizam QR Code existirá o QR Code Composto que dispõe de um rol de funcionalidades abrangentes, além das já existentes no QR Code estático no QR Code dinâmico. O que diferencia esse de QR Code é a existência de uma URL adicional, em um campo específico, cujo payload contém as informações relativas aos aspectos de configurações de recorrência de pagamentos oferecido pelo usuário recebedor ao usuário pagador.
...
|------------------|----------------------------------------------------------------------------|---------------|
| NÃO_PERMITE      | Não será autorizado retentativas após o vencimento                        | • /rec (POST) |
| PERMITE_3R_7D    | Será permitida até 3 tentativas após o vencimento dentro de 7 dias corridos | • /rec (POST) |
...
| valorMinimoRecebedor (valor)   | Campo opcional. Valor definido pelo recebedor como piso mínimo da autorização. Não pode ser inferior ao valor definido pelo pagador. Não deve ser preenchido quando o valor for fixo.                                                                                                                                                    | Não         | –       | String       | 100.00 — Pattern: `\d{1,10}\.\d{2}`           |
| politicaRetentativa            | Política de retentativas pós-vencimento. Define se haverá tentativas após a data limite. Opções: NAO_PERMITE; PERMITE_3R_7D (até três tentativas em sete dias corridos).                                                                                                                                                                | Sim         | –       | String ENUM  | NAO_PERMITE                                   |
| loc                            | Identificador da *location* a ser informada na criação da recorrência.                                                                                                                                                                                                                                                                  | Sim         | –       | Object       | –                                             |
...
| dadosJornada (ativacao)        | Dado obrigatório para a Jornada três. Deve ser removido pelo PSP recebedor nas jornadas um, dois ou quatro.                                                                                                                                                                                                                             | Não         | –       | Object       | JORNADA_3                                     |
| txid (dadosJornada)            | Identificador da transação. Permite conciliação entre PSP recebedor e usuário recebedor. No pacs.008, mapeado para TransactionIdentification `<txid>` ou idConciliacaoRecebedor.                                                                                                                 | Não         | –       | String       | 33beb661bed4a4892feff47dbeb2cd5 — Pattern: `[a-zA-Z0-9]{26,35}` |
