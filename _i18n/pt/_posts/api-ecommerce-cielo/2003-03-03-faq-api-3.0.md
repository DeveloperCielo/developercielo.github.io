...
No campo "**Usage**" você deverá indicar se é a primeira transação utilizando um cartão que ficará armazenado para ser usado futuramente (seja em compras pontuais ou recorrências) ou se é uma transação usando credenciais armazenadas anteriormente. 

- Caso seja a primeira transação o valor deve ser "**First**". 

- Caso seja uma transação usando credenciais armazenadas que já foram utilizadas em uma transação anterior, o valor deverá ser "**Used**".
...
Obs.: para compras eventuais e/ou pontuais com credenciais armazenadas e inciadas pelo portador do cartão (modelo CIT - Cardholder Initiated Transaction), o campo "Reason" não será utilizado.
