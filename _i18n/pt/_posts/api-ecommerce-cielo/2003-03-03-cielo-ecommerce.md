...
Para executar as operações da API E-commerce Cielo você deverá usar suas credenciais (`MerchantId` e `MerchantKey`) nos respectivos endpoints dos ambientes:
...
> [Faça o download do tutorial]({{ site.baseurl }}/attachment/merchantid-merchantkey-cielo-2023.pdf){:target="\_blank"} para saber como gerar as credenciais **MerchantId** e **MerchantKey** no [portal da Cielo](https://www.cielo.com.br/){:target="\_blank"}.
...
- [Certificado Raiz]({{ site.baseurl }}/attachment/root-2024.zip)
- [Certificado Intermediária]({{ site.baseurl }}/attachment/intermediate-2024.zip)
- [Certificado E-Commerce Cielo]({{ site.baseurl }}/attachment/cielo-ecommerce-2024.zip)
...
| `MerchantOrderId`                     | texto        | 50      | Sim                 | Número de identificação do pedido. Atenção: Não são permitidos os caracteres especiais. |
...
| `Customer.Birthdate`                  | data         | 10      | Não                 | Data de nascimento do comprador (AAAA/MM/DD).                                                                                                                                                                                                                                                                   |
...
| `Payment.CreditCard.Brand`                    | texto        | 10      | Sim                 | Bandeira do cartão. Valores possíveis: Visa / Master / Amex / Elo / Aura / JCB / Diners / Discover / Hipercard.                                                                                                                                                                            |
...
| `MerchantOrderId`                            | Número de identificação do Pedido. Atenção: Não são permitidos os caracteres especiais > (maior), < (menor), ! (ponto de exclamação), % (porcentagem), \ (barra inversa) e & (E comercial).                                                                                                                            | Texto    | 50      | Sim                 |
...
| `MerchantOrderId`                     | texto        | 50      | Sim                 | Número de identificação do Pedido. Atenção: Não são permitidos os caracteres especiais > (maior), < (menor), ! (ponto de exclamação), % (porcentagem), \ (barra inversa) e & (E comercial).                                                                                                                                                                                                                                                                |
| `Customer.Name`                       | texto        | 255     | Não                 | Nome do comprador.                                                                                                                                                                                                                                                                                 |
| `Customer.Status`                     | texto        | 255     | Não                 | Status de cadastro do comprador na loja (NEW / EXISTING)                                                                                                                                                                                                                                           |
...
| `Payment.QrCodeString`          | Texto codificado para o comprador "copiar" e "colar" no campo do internet banking em pagamentos feitos no ambiente mobile. A disponibilização deste código é de responsabilidade do lojista. Recomendamos a exibição de um botão que, ao clicar, o comprador copie o código.                                                                                      | texto  | Variável | Texto alfanumérico                   |
...
| `RequestId`                     | Identificador do Request, utilizado quando o lojista usa diferentes servidores para cada GET/POST/PUT                                  | GUID   | 36                                  | Não         |
| `MerchantOrderId`               | Número de identificação do Pedido. Atenção: Não são permitidos os caracteres especiais > (maior), < (menor), ! (ponto de exclamação), % (porcentagem), \ (barra inversa) e & (E comercial).                                                                                                                                                                | texto  | Bradesco: 27<BR>Banco do Brasil: 50 | Sim         |
...
| `Payment.Type`                  | Tipo do meio de pagamento.                                                                                                             | texto  | 100                                 | Sim         |
| `Payment.Amount`                | Valor do Pedido (ser enviado em centavos).                                                                                             | número | 15                                  | Sim         |
| `Payment.Provider`              | Define comportamento do meio de pagamento. Valores possíveis: "Bradesco2" ou "BancoDoBrasil2" | texto  | 15                                  | Sim         |
...
| `Content-Type`         | header   | 40      | Sim         | application/json (obrigatório o envio deste).                                                           |
| `RequestId`            | GUID     | 36      | Não         | Identificador do Request, utilizado quando o lojista usa diferentes servidores para cada GET/POST/PUT.  |
| `MerchantOrderId`      | texto    | 50      | Sim         | Numero de identificação do Pedido. Atenção: Não são permitidos os caracteres especiais > (maior), < (menor), ! (ponto de exclamação), % (porcentagem), \ (barra inversa) e & (E comercial).                                                                      |
| `Customer.Name`        | texto    | 255     | Não         | Nome do comprador.                                                                                      |
...
- **Mais segurança:** além da criação de um código (token ou DPAN) para substituir a informação do cartão, as bandeiras também emitem os criptogramas, que funcionam como uma senha ou assinatura da bandeira, única para aquele cartão naquele estabelecimento;
...
| `RequestId`                 | Identificador do Request, utilizado quando o lojista usa diferentes servidores para cada GET/POST/PUT                          | Guid    | 36      | Não         |
| `MerchantOrderId`           | Numero de identificação do Pedido. Atenção: Não são permitidos os caracteres especiais > (maior), < (menor), ! (ponto de exclamação), % (porcentagem), \ (barra inversa) e & (E comercial).                                                                                             | Texto   | 50      | Sim         |
...
| `Customer.Birthdate`                  | Data de nascimento do comprador (AAAA/MM/DD).                                                                      | Date  | 10      | Não         |
| `Customer.Identity`                   | Número do RG, CPF ou CNPJ do comprador.                                                                 | Texto | 14      | Não         |
...
Para consultar uma venda através do PaymentId, siga a requisição de exemplo a seguir.
...
| Propriedade                          | Descrição                                                                                                                                                                                                                                                                                                                                | Tipo               | Tamanho | Formato                              |
| ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ | ------- | ------------------------------------ |
| `MerchantOrderId`                    | Número de identificação do pedido. Atenção: Não são permitidos os caracteres especiais > (maior), < (menor), ! (ponto de exclamação), % (porcentagem), \ (barra inversa) e & (E comercial).                                                                                                                                                                                                                                                                                                       | Texto              | 50      | Texto alfanumérico                   |
...
| `Payment.EndToEndId` | Número de identificação único de uma transação Pix. |Texto | 50 | Texto alfanumérico |
...
| `CreditCard.Brand`                   | Bandeira do cartão (Visa / Master / Amex / Elo / Aura / JCB / Diners / Discover / Hipercard).                                                                                                                                                                                                                                    | Texto              | 10      | Texto                                |
...
| `MerchantKey`     | Chave Publica para Autenticação Dupla na API Cielo eCommerce.                                         | Texto | 40      | Sim         |
| `RequestId`       | Identificador do Request, utilizado quando o lojista usa diferentes servidores para cada GET/POST/PUT | Guid  | 36      | Não         |
| `MerchantOrderId` | Campo identificador do pedido na loja. Atenção: Não são permitidos os caracteres especiais > (maior), < (menor), ! (ponto de exclamação), % (porcentagem), \ (barra inversa) e & (E comercial).                                                               | Texto | 36      | Sim         |
...
2. Envie a requisição do Zero Auth para validar o cartão.
...
* Para melhor desempenho, é importante informar o CVV;
* Não há custo para a loja.
...
| 130    | Could not get Credit Card                                                                                      |Não é possível encontrar um cartão pelo cardtoken enviado                                                                                           |
