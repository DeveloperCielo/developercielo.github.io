...
O recurso **POST /payments** é responsável por efetuar o pagamento com base nos dados contidos no payload da requisição. No header da requisição é necessário os dados de **client_id**, **access_token**, **Content-type** e **wallet**. Wallet representa a carteira digital da aplicação sendo única por app. O corpo da requisição é composto por dados provenientes do aplicativo e do QR Code. [Veja mais](https://developercielo.github.io/manual/qrcode#public-keys) 
